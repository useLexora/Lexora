...
const mount = computed(() => {
  ...
  const element = target ? mountPoints.value.get(target) : null
  return target && element ? { target, element } : null
})
const input = computed<ExtensionViewInput | null>(() => {
  const parsed = extensionViewInputSchema.safeParse({ viewId: props.view.id, extensionId: props.view.resource.data.extensionId, viewType: props.view.type, placementId: props.view.resource.data.placementId, resource: props.view.resource.data.resource ?? null, state: props.view.state.value ?? {}, stateVersion: props.view.state.version ?? 0 })
